...
RSpec.describe 'Api::V1::Accounts::AiAgent::Documents', type: :request do
  ...
  let(:topic2) { create(:ai_agenttopic, account: account) }
  ...
  describe 'GET /api/v1/accounts/:account_id/ai_agent/documents' do
    ...
    context 'when it is an agent' do
      context 'when no filters are applied' do
        before do
          create_list(:ai_agentdocument, 30, topic: topic, account: account)
        end
      ...
      context 'when filtering by topic_id' do
        before do
          create_list(:ai_agentdocument, 3, topic: topic, account: account)
          create_list(:ai_agentdocument, 2, topic: topic2, account: account)
        end
        ...
        it 'returns empty array when topic has no documents' do
          new_topic = create(:ai_agenttopic, account: account)
          get "/api/v1/accounts/#{account.id}/ai_agent/documents",
      ...
      context 'when documents belong to different accounts' do
        ...
        before do
          create_list(:ai_agentdocument, 3, topic: topic, account: account)
          create_list(:ai_agentdocument, 2, account: other_account)
        end
      ...
      context 'with pagination and topic filter combined' do
        before do
          create_list(:ai_agentdocument, 30, topic: topic, account: account)
          create_list(:ai_agentdocument, 10, topic: topic2, account: account)
        end
  ...
  describe 'POST /api/v1/accounts/:account_id/ai_agent/documents' do
    ...
    context 'when it is an admin' do
      ...
      context 'with limits exceeded' do
        before do
          create_list(:ai_agentdocument, 5, topic: topic, account: account)
  ...
  describe 'DELETE /api/v1/accounts/:account_id/ai_agent/documents/:id' do
    ...
    context 'when it is an agent' do
      let!(:document_to_delete) { create(:ai_agentdocument, topic: topic) }
