...
RSpec.describe Account, type: :model do
  ...
  context 'with usage_limits' do
    ...
    describe 'when aiAgent limits are configured' do
      before do
        create_list(:ai_agentdocument, 3, account: account, topic: topic, status: :available)
        create(:installation_config, name: 'AI_AGENT_CLOUD_PLAN_LIMITS', value: ai_agentlimits.to_json)
      end

      ## Document
      it 'updates document count accurately' do
        account.update_document_usage
        expect(account.custom_attributes['ai_agentdocuments_usage']).to eq(3)
      end

      it 'handles zero documents' do
        account.ai_agentdocuments.destroy_all
        account.update_document_usage
        expect(account.custom_attributes['ai_agentdocuments_usage']).to eq(0)
      end
      ...
      it 'reflects document limits' do
        ...
        expect(document_limits[:current_available]).to eq ai_agentlimits[:startups][:documents] - 3
      end
      ...
      it 'incrementing responses updates usage_limits' do
        ...
        expect(account.custom_attributes['ai_agentresponses_usage']).to eq 1
        expect(responses_limits[:consumed]).to eq 1
        expect(responses_limits[:current_available]).to eq ai_agentlimits[:startups][:responses] - 1
      end

      it 'reseting responses limits updates usage_limits' do
        account.custom_attributes['ai_agentresponses_usage'] = 30
        account.save!

        responses_limits = account.usage_limits[:ai_agent][:responses]

        expect(responses_limits[:consumed]).to eq 30
        expect(responses_limits[:current_available]).to eq ai_agentlimits[:startups][:responses] - 30

        account.reset_response_usage
        responses_limits = account.usage_limits[:ai_agent][:responses]

        expect(account.custom_attributes['ai_agentresponses_usage']).to eq 0
        expect(responses_limits[:consumed]).to eq 0
        expect(responses_limits[:current_available]).to eq ai_agentlimits[:startups][:responses]
      end
      ...
      it 'returns monthly limit accurately' do
        %w[startups business enterprise].each do |plan|
          ...
          account.save!
          expect(account.ai_agentmonthly_limit).to eq ai_agentlimits[plan]
        end
      end

      it 'current_available is never out of bounds' do
        account.custom_attributes['ai_agentresponses_usage'] = 3000
        account.save!
        ...
        account.custom_attributes['ai_agentresponses_usage'] = -100
        account.save!
    ...
    describe 'when aiAgent limits are not configured' do
      it 'returns default values' do
        ...
        expect(account.ai_agentmonthly_limit).to eq(
    ...
    describe 'when limits are configured for an account' do
      before do
        create(:installation_config, name: 'AI_AGENT_CLOUD_PLAN_LIMITS', value: ai_agentlimits.to_json)
        account.update(limits: { ai_agentdocuments: 5555, ai_agentresponses: 9999 })
      end
